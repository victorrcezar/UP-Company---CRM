...
const DashboardSkeleton = () => (
    <div className="space-y-8 animate-pulse">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {[1,2,3,4].map(i => (
                <div key={i} className="h-40 skeleton rounded-[2rem]"></div>
            ))}
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 h-96 skeleton rounded-[2.5rem]"></div>
            <div className="h-96 skeleton rounded-[2.5rem]"></div>
        </div>
        <div className="h-64 skeleton rounded-[2.5rem]"></div>
    </div>
);

const StatCard = ({ title, value, trend, icon: Icon, color, delay }: any) => (
  <div 
    className="bg-white dark:bg-up-surface p-6 rounded-[2rem] shadow-sm hover:shadow-glow border border-slate-100 dark:border-slate-800/50 flex flex-col justify-between hover:-translate-y-1 transition-all duration-300 group cursor-default opacity-0 animate-slide-up"
    style={{ animationDelay: `${delay}ms` }}
  >
    <div className="flex justify-between items-start">
      <div>
        <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">{title}</p>
        <h3 className="text-3xl font-black text-slate-900 dark:text-white mt-3 tracking-tight">{value}</h3>
      </div>
      <div className={`p-3.5 rounded-2xl ${color} bg-opacity-10 text-opacity-100 ring-4 ring-transparent group-hover:ring-current group-hover:ring-opacity-5 transition-all duration-500`}>
        <Icon size={24} className="text-current" />
      </div>
    </div>
    <div className="mt-6 flex items-center text-[10px] font-black uppercase tracking-widest">
      <span className="text-green-500 flex items-center bg-green-50 dark:bg-green-900/20 px-2 py-0.5 rounded-lg"><ArrowUpRight size={14} className="mr-1" />{trend}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white dark:bg-up-surface p-8 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800/50 opacity-0 animate-slide-up" style={{ animationDelay: '500ms' }}>
          <div className="flex justify-between items-center mb-10">
            ...
            <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase bg-slate-50 dark:bg-slate-900 px-3 py-1 rounded-lg">
      ...
      <div className="bg-white dark:bg-up-surface p-10 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800/50 opacity-0 animate-slide-up" style={{ animationDelay: '700ms' }}>
        <div className="flex justify-between items-center mb-10">
            ...
            <button onClick={() => navigate('/agenda')} className="flex items-center gap-2 text-[10px] font-black text-up-accent uppercase tracking-widest hover:underline group">
                Ver Agenda Completa <ArrowRight size={12} className="group-hover:translate-x-1 transition-transform" />
            </button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {followUps.length > 0 ? followUps.map((lead, idx) => (
                <div 
                    ...
                    className="group flex items-center gap-5 p-5 bg-slate-50 dark:bg-slate-900/40 rounded-[2rem] border border-transparent hover:border-blue-500/30 hover:bg-white dark:hover:bg-slate-800 hover:shadow-xl hover:-translate-y-1 cursor-pointer transition-all duration-300"
                    style={{ animationDelay: `${750 + (idx * 50)}ms` }}
                >
                    <div className="w-14 h-14 rounded-2xl bg-white dark:bg-up-surface flex flex-col items-center justify-center border border-slate-100 dark:border-slate-800 shadow-sm shrink-0 group-hover:scale-110 group-hover:border-blue-500/20 transition-all">
                        ...
                        <span className="text-xl font-black text-slate-900 dark:text-white leading-none mt-1">{new Date(lead.nextFollowUp!).getDate()}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest mt-1 truncate">{lead.company || lead.source}</p>
                    </div>
                    <div className="w-8 h-8 rounded-full bg-white dark:bg-slate-700 flex items-center justify-center text-slate-300 dark:text-slate-500 group-hover:bg-blue-500 group-hover:text-white transition-all">
