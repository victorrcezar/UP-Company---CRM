...
import { Plus, DollarSign, AlertTriangle, Edit, Clock, Search, ChevronRight, UserCheck, GripVertical, Filter } from 'lucide-react';
...
const Pipeline = () => {
  ...
  return (
    // Altura calculada para ocupar exatamente o espaço disponível entre o header e o fim da tela, considerando paddings
    <div className="flex flex-col h-[calc(100vh-140px)] md:h-[calc(100vh-180px)] animate-fade-in overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 px-1 shrink-0">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="relative flex-1 md:flex-none group">
                ...
                <input 
                    type="text" 
                    placeholder="Filtrar funil..." 
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-up-surface border border-slate-200 dark:border-slate-700/50 rounded-xl text-xs font-bold outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all md:w-64 dark:text-white shadow-sm"
                />
            </div>
            <button 
                onClick={() => navigate('/leads', { state: { openCreateModal: true } })}
                className="bg-slate-900 dark:bg-up-accent dark:text-up-dark text-white px-5 py-2.5 rounded-xl text-xs font-black hover:opacity-90 transition-all uppercase tracking-widest shadow-lg hover:shadow-xl flex items-center gap-2 whitespace-nowrap active:scale-95"
            >
        ...
        <div className="flex gap-4 h-full min-w-max px-1">
          {PIPELINE_COLUMNS.map((col) => {
            ...
            return (
              <div 
                key={col.id}
                className={`w-[320px] md:w-[350px] flex flex-col rounded-[1.5rem] transition-all duration-300
                    ${isDragOver 
                        ? 'ring-2 ring-blue-500 bg-blue-50/30 dark:bg-blue-900/10' 
                        : 'bg-slate-100/50 dark:bg-slate-900/20 border border-transparent hover:border-slate-200/50 dark:hover:border-slate-700/30'
                    }
                `}
              ...
              >
                ...
                <div className="p-4 pb-2 shrink-0">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-black text-[11px] text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                        <span className={`w-2.5 h-2.5 rounded-full ${col.barColor} shadow-sm`}></span>
                        {col.title}
                    </h3>
                    <span className="bg-white dark:bg-slate-800 px-2 py-0.5 rounded-lg text-[10px] font-black text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 shadow-sm">
                      {columnLeads.length}
                    </span>
                  </div>
                  {totalValue > 0 && (
                      <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/50 dark:bg-slate-800/50 rounded-lg border border-slate-200/50 dark:border-slate-700/50 w-fit mb-1">
                          <DollarSign size={12} className="text-green-600 dark:text-green-500" />
                          <span className="text-[10px] font-black text-slate-700 dark:text-slate-300 tracking-tight">
                              {totalValue.toLocaleString('pt-BR', { notation: "compact", maximumFractionDigits: 1 })}
                          </span>
                      </div>
                  )}
                  {/* Progress Bar Background */}
                  <div className="w-full h-1 bg-slate-200 dark:bg-slate-700 rounded-full mt-2 overflow-hidden">
                      <div className={`h-full ${col.barColor} opacity-70`} style={{ width: `${Math.min((columnLeads.length / 10) * 100, 100)}%` }}></div>
                  </div>
                </div>
                ...
                <div className="flex-1 px-3 pb-3 overflow-y-auto space-y-3 custom-scrollbar">
                  {loading ? (
                       [1,2].map(i => <div key={i} className="h-28 rounded-2xl bg-slate-200/50 dark:bg-slate-800/50 animate-pulse" />)
                  ) : (
                    columnLeads.map((lead, idx) => {
                        ...
                        return (
                            <div
                                ...
                                className={`group relative bg-white dark:bg-up-surface p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800/50 cursor-grab active:cursor-grabbing transition-all hover:shadow-xl hover:-translate-y-1 hover:border-blue-500/30 animate-scale-up`}
                                style={{ animationDelay: `${idx * 50}ms` }}
                            >
                                <div className="flex justify-between items-start mb-2">
                                    <span className="px-2 py-0.5 rounded-md bg-slate-50 dark:bg-slate-900 text-[9px] font-black text-slate-400 uppercase tracking-widest border border-slate-100 dark:border-slate-800">
                                        #{lead.id.slice(-4)}
                                    </span>
                                    {isStagnant && (
                                        <div className="flex items-center gap-1 text-[9px] font-bold text-orange-500 bg-orange-50 dark:bg-orange-900/10 px-1.5 py-0.5 rounded-md animate-pulse">
                                            <AlertTriangle size={10} /> +10 dias
                                        </div>
                                    )}
                                </div>
                                
                                <h4 className="font-bold text-slate-800 dark:text-white text-sm mb-1 leading-snug line-clamp-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                                    {lead.name}
                                </h4>
                                <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3 truncate">
                                ...
                                <div className="flex justify-between items-center pt-3 border-t border-slate-50 dark:border-slate-800/50">
                                    <span className="text-xs font-black text-slate-700 dark:text-slate-200 tracking-tight flex items-center gap-1">
                                ...
                                <div className="absolute inset-0 bg-slate-900/90 dark:bg-black/80 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-all duration-200 rounded-2xl flex flex-col items-center justify-center gap-3 z-10 pointer-events-none group-hover:pointer-events-auto transform scale-95 group-hover:scale-100">
